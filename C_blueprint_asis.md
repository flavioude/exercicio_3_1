...
| **Processos de Suporte** | Portal institucional do CBMDF / servidor de arquivos de ITs | SCIPWeb (módulo de cadastro) | SCIPWeb (módulo de peticionamento) + sistema de arrecadação/compensação bancária do GDF | ⚠️ Módulo de Triagem do DESEGWEB | ⚠️ DESEGWEB (visualizador técnico de arquivos DWF) | ⚠️ Módulo de Recursos/Gestão de Atas do CTDSCI | SCIPWeb + DESEGWEB (reenvio e checagem) | ⚠️ Sistema assinador ICP-Brasil / emissor de QR Code da DESEG |
